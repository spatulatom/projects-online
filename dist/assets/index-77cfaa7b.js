(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))n(t);new MutationObserver(t=>{for(const a of t)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&n(i)}).observe(document,{childList:!0,subtree:!0});function m(t){const a={};return t.integrity&&(a.integrity=t.integrity),t.referrerPolicy&&(a.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?a.credentials="include":t.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function n(t){if(t.ep)return;t.ep=!0;const a=m(t);fetch(t.href,a)}})();const g=document.getElementById("light"),f=document.getElementById("dark"),d=document.getElementById("system"),y=document.getElementById("light-mobile"),x=document.getElementById("dark-mobile"),l=document.getElementById("system-mobile"),o=document.querySelector(".dropdown");function r(){g.classList.remove("text-cyan"),f.classList.remove("text-cyan"),d.classList.remove("text-cyan"),y.classList.remove("text-cyan"),x.classList.remove("text-cyan"),l.classList.remove("text-cyan")}"color-theme"in localStorage?localStorage.getItem("color-theme")==="dark"||!("color-theme"in localStorage)&&window.matchMedia("(prefers-color-scheme: dark)").matches?"mode-system"in localStorage?(r(),d.classList.add("text-cyan"),o.innerHTML=`<i
    ...
    class="fa-solid fa-sun fa-xl cursor-pointer text-cyan"
  >
  </i>`);const E=window.matchMedia("(prefers-color-scheme: dark)");E.addEventListener("change",({matches:e})=>{console.log("EEEEEEVENTTTT listener triggered."),r(),d.classList.add("text-cyan"),l.classList.add("text-cyan"),e?(localStorage.setItem("color-theme","dark"),localStorage.setItem("mode-system","system"),document.documentElement.classList.add("dark"),o.innerHTML=`<i
      id="dark"
      class="fa-solid fa-moon fa-xl cursor-pointer transition-all text-cyan"
    ></i>`,console.log("change to dark mode!")):(console.log("change to light mode!"),localStorage.setItem("color-theme","light"),localStorage.setItem("mode-system","system"),document.documentElement.classList.remove("dark"),o.innerHTML=`<i
      class="fa-solid fa-sun fa-xl cursor-pointer text-cyan"
    >
    </i>`)});f.addEventListener("click",L);g.addEventListener("click",k);d.addEventListener("click",S);x.addEventListener("click",L);y.addEventListener("click",k);l.addEventListener("click",S);function L(){r(),f.classList.add("text-cyan"),o.innerHTML=`<i
  id="dark"
  class="fa-solid fa-moon fa-xl cursor-pointer transition-all text-cyan"
></i>`,x.classList.add("text-cyan"),document.documentElement.classList.add("dark"),localStorage.setItem("color-theme","dark"),localStorage.removeItem("mode-system")}function k(){r(),g.classList.add("text-cyan"),o.innerHTML=`<i
  class="fa-solid fa-sun fa-xl cursor-pointer text-cyan"
>
</i>`,y.classList.add("text-cyan"),document.documentElement.classList.remove("dark"),localStorage.setItem("color-theme","light"),localStorage.removeItem("mode-system")}function S(){r(),d.classList.add("text-cyan"),l.classList.add("text-cyan"),window.matchMedia("(prefers-color-scheme: dark)").matches?(localStorage.setItem("color-theme","dark"),localStorage.setItem("mode-system","system"),document.documentElement.classList.add("dark"),o.innerHTML=`<i
    id="dark"
    class="fa-solid fa-moon fa-xl cursor-pointer transition-all text-cyan"
