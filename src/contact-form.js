    ...
    if (object.email !== object.email2) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setTimeout(() => {
            results.innerHTML =
              'You will get an confirmation email from tom.sypula@gmail.com (it might be in the spam folder). I will reply back to you within 24h. Thank you and have a nice day!';
          }, 6000);
        } else {
          const jsonResponse = await response.json();
