    ...
    if (object.email !== object.email2) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setTimeout(() => {
            results.innerHTML =
              'You will receive a confirmation email (it might be in the spam folder) titled: Tom Spatula.  I will reply back to you within 24 hours. Thank you and have a nice day!';
          }, 6000);
        } else {
          const jsonResponse = await response.json();
